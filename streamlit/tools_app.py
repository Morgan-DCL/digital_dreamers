...
async def fetch_persons_bio(
    ...
    async with aiohttp.ClientSession() as ss:
        ...
        for data in datas:
            ...
            if director:
                top_credits = sorted(
                    (
                        ...
                        and n["job"] == "Director"
                        and all(
                            genre not in n["genre_ids"] for genre in exclude
                        )
                    ),
                    key=lambda x: (
                        -x["popularity"],
            ...
            cmt = 'streamlit/img/blank_img.jpg'
            data["image"] = (
...
async def fetch_persons_movies(ids: int, people_list: list) -> dict:
    async with aiohttp.ClientSession() as ss:
        ...
        for data in datas:
            ...
            data["characters"] = {
                n["id"]: n["character"] for n in test if n["id"] in people_list
            }
...
def knn_algo(df: pd.DataFrame, titre: str, top: int = 5) -> list:
    ...
    _, indices = knn_model.kneighbors(count_matrix[index], n_neighbors=top + 1)
    result = []
...
def get_clicked_bio(api_list: list, dup_ids: dict, nb: int):
    ...
    character = (
        [n for n in api_list["character"]][nb] if not peo["director"] else ""
    )
